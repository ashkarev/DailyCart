...
const CartSlice = createSlice({
  ...
  reducers: {
    addtoCart: (state, argToAdd) => {
      let existingData = state.CartData.find(
        (eachPro) => eachPro.id == argToAdd.payload.id,
      );
      if (existingData) {
        existingData.quantity++;
        existingData.totalPrice = existingData.quantity * existingData.price;

        let remainingProduct = state.CartData.filter(
          (dataEach) => dataEach.id != existingData.id,
        );
        ...
        state.CartData = [
          ...state.CartData,
          {
            ...argToAdd.payload,
            quantity: 1,
            totalPrice: argToAdd.payload.price,
          },
        ];
    ...
    reduceFromCart: (state, removeCart) => {
      let existingData = state.CartData.find(
        (state) => state.id == removeCart.payload.id,
      );
      if (existingData) {
        existingData.quantity--;
        existingData.totalPrice = existingData.quantity * existingData.price;
...
export const { addtoCart, reduceFromCart, deleteCart, emptyCart } =
  CartSlice.actions;
